...
function VirtualFs({
  ...
  fixturesPath,
  // resetGlob = '*', // only supported with real fs
}) {
  ...
  const reset = async files => {
    ...
    const newFiles = await mfs.reset(mergedFiles)
    if (notify) {
      const changes = unique([...newFiles, ...oldFiles])
  ...
  return Object.assign(ufs, {
    ...
    lstatSync,
    statSync: lstatSync,
    close: noop, // unionfs.close causes weird error
  })
}
