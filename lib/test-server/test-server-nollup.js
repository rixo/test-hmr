...
const start = async arg => {
  const {
    ...
    rollupConfig = require(`${appPath}/rollup.config.js`),
  } = resolveArgs(arg)
  ...
  const svelteHmr = rollupConfig.plugins.find(
    ({ name }) => name === 'svelte-hmr'
  )
  svelteHmr._setFs(vfs)
