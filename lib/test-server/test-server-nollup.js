...
const start = async arg => {
  ...
  const nollupPath = require.resolve('nollup/lib/dev-middleware', require.main)

  const nollupDevServer = proxyquire.noCallThru().load(nollupPath, {
  ...
  const hotPlugin =
    ...
    ) || rollupConfig.plugins.find(({ name }) => name === 'svelte')
