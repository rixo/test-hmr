...
const consumeSub = (sub, ...args) => consume(sub.call(commands), ...args)
...
const assertExpect = async (state, expectation, cond) => {
  ...
  try {
    if (before) {
      stepName = 'before'
      await consumeSub(before, state.processCommand)
    }
    let i = 0
    for (const step of steps) {
      ...
      await passIt(its, index, async () => {
        ...
        if (beforeStep) {
          ...
          await consumeSub(beforeStep, state.processCommand)
        }
        if (fn) {
        ...
        if (sub) {
          stepName = `step ${index} (sub)`
          await consumeSub(sub, state.processCommand)
        }
        if (html != null) {
        ...
        if (afterStep) {
          ...
          await consumeSub(afterStep, state.processCommand)
        }
      })
    }
    if (after) {
      stepName = 'after'
      await consumeSub(after, state.processCommand)
    }
...
const processTemplates = (state, cmd) => {
  Object.assign(state.templates, cmd.templates)
}
...
const initCommandProcessor = (state, start) => async command => {
  debug('init command', command)
  switch (command.type) {
    case DEBUG:
      return state
...
const commandProcessor = state => {
  ...
  return async command => {
    ...
    switch (command.type) {
      ...
      case PAGE:
        return processPageProxy(state, command)
...
const runHandler = async (config, handler) => {
  ...
  const start = async firstCommand => {
    ...
    const beforeGoto =
      ...
      (page => {
        state.page = page
        return consumeSub(state.beforeLoad, processCommand)
      })

    await loadPage(state.pageUrl, inPage, beforeGoto)
  }
  ...
  const started = !state.started && state.expects.size > 0
  debug('runHandler (started=%s)', started)
  if (started) {
    await start()
  }
}
