...
const startPuppeteer = async ({ open }) => {
  ...
  const opts = open
    ? {
        ...
        slowMo: 100,
        devtools: true,
        // userDataDir: '/tmp/pptr.rixo', // DEBUG DEBUG DEBUG
      }
    : {}
...
const setupTestServerDefault = ({ start }) => () => {
  debug('setupDefault')
  let app
  beforeEach(async () => {
...
const setupTestServerFast = ({ start }, { keepRunning }) => () => {
...
module.exports = ({ appPath = config.appPath } = {}) => {
  ...
  const sveltePath = !appPath
    ? null
    : path.resolve(appPath, 'node_modules', 'svelte')

  if (fs.existsSync(appPath)) {
    const { version } = require(path.resolve(sveltePath, 'package.json'))
    ...
    // eslint-disable-next-line no-console
    console.log(`[Test HMR] Svelte v${version}: ${sveltePath}`)
  } else {
    // eslint-disable-next-line no-console
