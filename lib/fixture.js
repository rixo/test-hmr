...
const root = global.ZOAR_OPTIONS.cwd
...
const createFixture = async (
  { adapterName: adapter, dir: fixturesDir = path.resolve(root, 'fixtures') },
  {
    base = 'default',
    source,
    encoding = 'utf8',
  ...
  for (const src of new Set(sources)) {
    if (typeof src === 'object') {
      ...
      // string
      const from = path.resolve(fixturesDir, src)
      ...
      if (!found) {
        throw new Error(
          `Failed to find fixture: ${src} (tried: ${froms.join(', ')})`
        )
      }
    }
  }
