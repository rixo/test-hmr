...
// ensures config survives mocha's reloads
const globalKey = Symbol.for('TEST_HMR_CONFIG')

const config = global[globalKey] || {
  set(cfg) {
  ...
  // use filesystem vs RAM fs mock
  fs: false,
  // directory from which are copied fixture files
  fixturesDir: null,
  // fixtures {[filename]: contents}
  fixtures: null,
  ...
  // open puppeteer's browser for debug (also slow things down)
  open: OPEN && OPEN != 0,
  keepOpen: false,
  break: true,

  // allows to remember & reuse user settings in puppeteer, like position of
  // the dev tools, etc. This is only used when open is true.
  userDataDir: null,
  ...
  // log browser's console to terminal
  console: false,

  // log bundler's output to the terminal
  logs: false,

  nollup: NOLLUP && NOLLUP != '0',

  hmrDoneMessage: '[HMR] App is up to date.',
  hmrReadyMessage: '[HMR] Status Change ready',
  hmrNothingChangedMessage: '[WDS] Nothing changed.',
  hmrCompileErrorMessage: '[WDS] Errors while compiling. Reload prevented.',
}

global[globalKey] = config
