...
import * as zorax from './harness.zorax.cjs'

const { plug } = zorax

let loadAdapter = async () => {
  const config = await getConfig()
  const { serve, hmr } = await import(`../adapters/${config.target}/index.js`)
  const loaded = {
    fixture: createFixture({
      adapterName: config.target,
      dir: config.fixturesDir,
    }),
...
const closable = (get, supervisor, beforeClose) => (opts) =>
  async function zora_spec_fn(t, next, ...args) {
    ...
    service.config = await getConfig()
    try {
...
const waitPageClose = async ({ page }) => {
  const config = await getConfig()
  if (config.keep) {
...
export const dev = closable(
  ...
  // Supervisors (monitoring early exit conditions)
  //
  ({ page, hmr, config }) => [
    // Timeout
    //
    ...
    new Promise((resolve, reject) => {
      page
        .exposeFunction('reportError', (err) => {
          reject(new Error(`Unexpected error: ${err}`))
        })
        .catch((err) => {
          if (/Target closed/.test(err)) return
          reject(err)
        })
    }),
...
export const { test, describe } = plug([
...

  {
    ...
    test(t) {
      t.isSnowpack = () => t.config.target === 'snowpack'
    },
  },
])
