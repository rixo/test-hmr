...
const compileFileContent = (shiftValues, { parts, conditions }) => {
  ...
  const allKey = conditions.length > 0 ? '*' : '0'
  const allLines = []
  const result = { [allKey]: allLines }
...
const splitValuesSubs = values => {
  const subs = []
  const others = []
  for (const val of values) {
    const sub = val.fn || (Array.isArray(val.obj) && val.obj)
    if (sub) subs.push({ ...val, sub })
...
const splitSteps = (part, values) => {
  ...
  const result = []
  let leftIndex = start
  const [subs, others] = splitValuesSubs(values)
...
const mapAst = (ast, values) => {
  ...
  if (ast.prolog) {
    const values = shiftValues(ast.prolog)
    for (const { fn } of values) {
      if (!fn) {
        throw new Error('Only sub functions are allowed in prolog')
      }
      result.beforeLoad.push(fn)
    }
  }

  if (ast.expectations) {
    let { expectations } = ast
    // case: if there are no explicit conditions in expected result, then
    // consider the whole expected string as condition ::0
    if (expectations.conditions.length === 0) {
      const { text, start, end } = expectations.parts[0]
      expectations = {
        ...expectations,
        conditions: ['0'],
        parts: [
    ...
    const { expects, beforeLoad } = compileSteps(shiftValues, expectations)
