...
const Post = () => {
  const callbacks = []
  const add = cb => callbacks.push(cb)
  const flush = (...args) => callbacks.forEach(cb => cb(...args))
  return { add, flush }
}
...
const ErrorManager = () => {
  ...
  const catchError = err => handlers.some(handler => handler(err))
  return { add, catch: catchError }
}

const catchPageError = (handlers, err) =>
  this.pageErrorHandlers.some(handler => handler(err))

const runHandler = async (config, handler) => {
  const { reset, loadPage } = config
  const post = Post()
  const pageErrors = ErrorManager()
  const state = {
    config,
    commands,
    pageUrl: '/',
    ...
    post: post.add,
    // catch errors
    addPageErrorHandler: pageErrors.add,
    catchPageError: pageErrors.catch,
  }
  ...
  const start = async firstCommand => {
    ...
    const inPage = async page => {
      ...
      try {
        ...
        await consume(state, gen, processCommand, firstValue)
  ...
  await consume(state, gen, processInitCommand)
