const debug = require('debug')('test-hmr:expect')
    ...
    for (const step of steps) {
      ...
      await passIt(its, index, async () => {
        ...
        if (html != null) {
          ...
          await assertHtml(state, html, stepName, index)
        }
        if (afterStep) {
...
const consumeExpects = async (state, _untilLabel, alreadyWritten = false) => {
  debug('consumeExpects(state, "%s", %s)', _untilLabel, alreadyWritten)
  const {
    config: { writeHmr },
  } = state
  ...
  while (remainingExpects.length > 0) {
    ...
    if (!alreadyWritten) {
      ...
      debug('consumeExpects: writeHmr(page, %s)', label)
